...
namespace MatesOfMate\PhpStanExtension\Tests\Unit\Cache;
...
class RunCacheTest extends TestCase
{
    ...
    {
        $this->cacheDir = sys_get_temp_dir().'/phpstan-run-cache-'.bin2hex(random_bytes(4));
    }
    ...
    public function testNoPartialFileIsLeftBehind(): void
    {
        ...
        $this->assertSame([], glob($this->cacheDir.'/phpstan-runs/.*.tmp') ?: []);
    }

    public function testStoringFailsLoudlyWhenTheDirectoryCannotBeCreated(): void
    {
        $cache = new RunCache('/proc/nope', 'phpstan-runs', 20);
    ...
    {
        return new RunCache($this->cacheDir, 'phpstan-runs', $keep);
    }
}
