...
namespace MatesOfMate\RectorExtension\Tests\Unit\Cache;
...
class RunCacheTest extends TestCase
{
    ...
    {
        $this->cacheDir = sys_get_temp_dir().'/rector-run-cache-'.bin2hex(random_bytes(4));
    }
    ...
    public function testNoPartialFileIsLeftBehind(): void
    {
        ...
        $this->assertSame([], glob($this->cacheDir.'/rector-runs/.*.tmp') ?: []);
    }

    public function testStoringFailsLoudlyWhenTheDirectoryCannotBeCreated(): void
    {
        $cache = new RunCache('/proc/nope', 'rector-runs', 20);
    ...
    {
        return new RunCache($this->cacheDir, 'rector-runs', $keep);
    }
}
